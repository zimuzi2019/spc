...
public class SheetT2Generator {
    public static byte[] generateT2(String graphType, Integer subgroupTotal, Integer subgroupCapacity, Integer varNum) {
        int rowNum = 0;
        int colNum = 0;

        XSSFWorkbook wb = new XSSFWorkbook();

        XSSFSheet sheet = wb.createSheet("多变量T^2控制图登入表");
        ...
        cell.setCellValue("多变量T^2控制图数据登入表");
        ...
        row = sheet.createRow(rowNum);
        cell = row.createCell(colNum);
        cell.setCellValue("变量个数");
        cell.setCellStyle(cellPropertyStyle);

        colNum = colNum + 3;
        cell = row.createCell(colNum);
        cell.setCellValue(varNum);
        cell.setCellStyle(cellValueStyle);
        ...
        rowNum++; colNum = 0;
        row = sheet.createRow(rowNum);
        for (int i = 1; i <= subgroupTotal; i++) {
            sheet.addMergedRegion(new CellRangeAddress(rowNum, rowNum+varNum-1, colNum, colNum+1));
            cell = row.createCell(colNum);
            cell.setCellValue(i);
            cell.setCellStyle(cellStyle1);
